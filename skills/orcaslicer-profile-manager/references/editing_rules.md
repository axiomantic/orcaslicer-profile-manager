...
The word "base" has two meanings in this domain. "Support base" means the bulk
support structure, the counterpart of the support interface. "Base profile" reads
as the plain single-material profile. The word cannot mean "parent preset"
either, because every user preset is flat and has no user parent.
...
- **Naming**: never use `Base` in a preset name. See § 1.1 for the replacement names.
